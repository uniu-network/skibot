<template>
      ...
      <n-space vertical size="large">
        <n-flex justify="space-between" align="center">
          ...
          <n-space align="center">
            ...
            <n-button secondary size="small" :loading="reloadingAll" @click="reloadAdapter()">
              重载全部
            </n-button>
          </n-space>
        </n-flex>
        ...
        <n-spin :show="loadingRef">
          <n-grid :cols="1" x-gap="12" y-gap="12">
            <n-gi v-for="adapter in adapters" :key="adapter.id">
              <n-card hoverable :title="adapter.name || adapter.id" size="small">
                <n-descriptions bordered :column="isPhone ? 1 : 3" label-placement="left">
                  <n-descriptions-item label="类型">{{ adapter.id }}</n-descriptions-item>
                  <n-descriptions-item label="状态">
                    <n-tag :type="adapter.loaded ? 'success' : 'warning'" size="small">
                ...
                <template #action>
                  <n-flex>
                    <n-button tertiary type="info" size="small" :loading="reloadingAdapterId === adapter.id" @click="reloadAdapter(adapter.id)">
                      重载
                    </n-button>
                    <n-button tertiary size="small" @click="openConfig(adapter)">
                      配置
                    </n-button>
                    <n-button tertiary type="error" size="small" @click="handleRemove(adapter)">
                      删除
                    </n-button>
                  </n-flex>
                </template>
          ...
          <n-empty v-if="adapters.length === 0 && !loadingRef" description="暂无适配器，点击上方按钮添加" style="margin-top: 50px;" />
        </n-spin>
      </n-space>
    </n-layout-content>
  </n-layout>
  ...
  <n-drawer v-model:show="configDrawerVisible" :width="560" placement="right">
    <n-drawer-content :title="`${currentAdapter?.name || currentAdapter?.id || ''} 配置`" closable>
      <n-spin :show="savingConfig">
        <n-dynamic-input v-model:value="configEntries" :on-create="createConfigEntry">
          <template #default="{ value }">
            <n-input v-model:value="value.key" placeholder="配置名" style="width: 42%; margin-right: 8px;" />
            <n-input v-model:value="value.value" placeholder="配置值" style="width: 55%;" />
          </template>
        </n-dynamic-input>
      </n-spin>
      <template #footer>
  ...
  <n-modal v-model:show="showAddModal" preset="dialog" title="添加适配器" positive-text="添加" negative-text="取消"
    :loading="addingAdapter" @positive-click="onAddAdapter">
    <n-space vertical>
      <n-select v-model:value="selectedAdapterType" :options="availableTypes" placeholder="选择适配器类型" @update:value="onAdapterTypeSelect" />

      <n-dynamic-input v-if="selectedAdapterType" v-model:value="addConfigEntries" :on-create="createConfigEntry">
        <template #default="{ value }">
          <n-input v-model:value="value.key" placeholder="配置名" style="width: 42%; margin-right: 8px;" />
          <n-input v-model:value="value.value" placeholder="配置值" style="width: 55%;" />
        </template>
...
async function loadAvailableTypes() {
  try {
    const resp = await axios.get('/api/adapters/available', { withCredentials: true })
...
async function reloadAdapter(type) {
  ...
  try {
    await axios.post('/api/adapters/reload', { botId: currentBotId.value, type }, { withCredentials: true })
...
function openConfig(adapter) {
  ...
  configEntries.value = Object.entries(adapter.config || {}).map(([key, value]) => ({
    key,
...
async function saveConfig() {
  ...
  try {
    await axios.post('/api/adapters/setConfig', {
...
function handleRemove(adapter) {
  dialog.warning({
    ...
    onPositiveClick: async () => {
      try {
        await axios.post('/api/adapters/remove', {
          botId: currentBotId.value,
          type: adapter.id,
        }, { withCredentials: true })
...
function onAdapterTypeSelect(val) {
  ...
  addConfigEntries.value = Object.entries(defaultConfig).map(([key, value]) => ({
    key,
    value: stringifyValue(value),
  }))
}
...
async function onAddAdapter() {
  ...
  try {
    await axios.post('/api/adapters/add', {
      botId: currentBotId.value,
    ...
    message.error(`添加失败: ${e.response?.data?.message || e.message || '未知错误'}`)
  } finally {
