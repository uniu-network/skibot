    ...
    <n-layout-content style="margin-top: 30px; margin-left: 30px; margin-right: 30px">
      <n-space vertical size="large">
        <n-flex justify="space-between" align="center">
          ...
          <n-space>
            <n-button type="primary" secondary :loading="reloadingAll" @click="reloadAll">
              重载全部
            </n-button>
          </n-space>
        </n-flex>
        ...
        <n-spin :show="loadingRef">
              ...
              <n-card hoverable :title="item.name" size="small">
                <template #header-extra>
                  <n-tooltip trigger="hover">
                    ...
                    {{ item.isEnabled && item.isLoaded ? '已启用' : (item.isEnabled === false ? '已禁用' : '未加载') }}
                  </n-tooltip>
                </template>
                <n-descriptions bordered :column="1" label-placement="left" size="small">
                  <n-descriptions-item label="描述">{{ item.description || '无描述' }}</n-descriptions-item>
                  <n-descriptions-item label="版本">{{ item.version || '-' }}</n-descriptions-item>
                  <n-descriptions-item label="作者">{{ item.author || '-' }}</n-descriptions-item>
          ...
          <n-empty v-if="data.length === 0 && !loadingRef" description="暂无插件" style="margin-top: 50px;" />
        </n-spin>
      </n-space>
    </n-layout-content>
  </n-layout>
  ...
  <n-drawer v-model:show="configDrawerVisible" :width="560" placement="right">
    <n-drawer-content :title="`配置 - ${currentPlugin?.name || ''}`" closable>
      <n-spin :show="configLoading">
        <n-space vertical>
          <template v-if="configSchema && configSchema.fields && configSchema.fields.length > 0">
            <template v-for="field in configSchema.fields" :key="field.key">
              <n-form-item :required="field.required">
                <template #label>
                  <div>
                    <span>{{ field.key }}</span>
                    <n-text v-if="field.description" depth="3" style="font-size: 12px; display: block;">{{ field.description }}</n-text>
                  </div>
                </template>
                <n-switch
...
async function togglePlugin(item: any, enabled: boolean) {
  ...
  try {
    ...
    message.success(enabled ? `插件 ${item.name} 已启用` : `插件 ${item.name} 已禁用`);
    await loadPlugins();
  } catch (e: any) {
...
async function reloadPlugin(item: any) {
  ...
  try {
    ...
    message.error(`重载失败: ${e.response?.data?.message || e.message || '未知错误'}`);
  } finally {
...
async function reloadAll() {
  ...
  try {
    ...
    message.error(`重载全部失败: ${e.response?.data?.message || e.message || '未知错误'}`);
  } finally {
...
async function unloadPlugin(item: any) {
  ...
  dialog.warning({
    ...
    onPositiveClick: async () => {
      try {
        await axios.post('/api/plugins/unload', {
          name: item.name,
          botId: currentBotId.value,
        }, { withCredentials: true });
        ...
        message.error(`卸载失败: ${e.response?.data?.message || e.message || '未知错误'}`);
      }
    },
  });
}
...
async function loadPlugin(item: any) {
  ...
  try {
    await axios.post('/api/plugins/load', {
      name: item.name,
      botId: currentBotId.value,
    }, { withCredentials: true });
...
async function openConfig(item: any) {
  ...
  try {
    const resp = await axios.post('/api/plugins/getConfig', {
      name: item.name,
      botId: currentBotId.value,
    }, { withCredentials: true });
    ...
    if (schema && schema.fields && schema.fields.length > 0) {
      for (const field of schema.fields) {
        if (field.type === 'object' && configData.value[field.key] !== undefined) {
          objectFields[field.key] = JSON.stringify(configData.value[field.key], null, 2);
        }
        if (configData.value[field.key] === undefined && field.default !== undefined) {
...
async function saveConfig() {
  ...
  try {
    ...
    if (configSchema.value && configSchema.value.fields && configSchema.value.fields.length > 0) {
      configToSave = { ...configData.value };
      ...
      for (const field of configSchema.value.fields) {
        ...
        if (field.type === 'number' && typeof configToSave[field.key] === 'string') {
      ...
      for (const entry of configEntries.value) {
        ...
        try { parsedValue = JSON.parse(entry.value); } catch (e) { /* keep as string */ }
        configToSave[key] = parsedValue;
      }
    }

    await axios.post('/api/plugins/setConfig', {
    ...
    message.error(`配置保存失败: ${e.response?.data?.errors?.join(', ') || e.message || '未知错误'}`);
  } finally {
    savingConfig.value = false;
  }
}
