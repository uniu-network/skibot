<template>
      ...
      <n-space vertical size="large">
        <n-flex justify="space-between" align="center">
          ...
          <n-space>
            ...
            <n-button secondary size="small" @click="loadBots"> 刷新 </n-button>
          </n-space>
        </n-flex>
        ...
        <n-spin :show="loadingRef">
          <n-grid :cols="1" x-gap="12" y-gap="12">
            <n-gi v-for="bot in botListData" :key="bot.botId">
              ...
              >
                ...
                >
                  <n-descriptions-item label="Bot ID">{{
                    bot.botId
                  }}</n-descriptions-item>
                  <n-descriptions-item label="名称">{{
                    bot.name
                  }}</n-descriptions-item>
                  <n-descriptions-item label="Self ID">{{
                    bot.self_id
                  }}</n-descriptions-item>
                  <n-descriptions-item label="前缀">
                    <n-flex size="small">
                      <n-tag
                        v-for="p in bot.prefix || ['/']"
                        :key="p"
                        size="small"
                        type="info"
                        >{{ p }}</n-tag
                  ...
                  <n-descriptions-item label="状态">
                    <n-tag
                      :type="bot.running ? 'success' : 'error'"
                      size="small"
                    >
                      {{ bot.running ? "运行中" : "已停止" }}
                    </n-tag>
                  </n-descriptions-item>
                  <n-descriptions-item label="插件数">{{
                    bot.pluginCount
                  }}</n-descriptions-item>
                  <n-descriptions-item label="适配器数">{{
                    bot.adapterCount
                  }}</n-descriptions-item>
                </n-descriptions>
                <template #action>
                  <n-flex>
                    ...
                    <n-button
                      ...
                      type="info"
                      size="small"
                      @click="handleReload(bot.botId)"
                      :loading="reloadingBotId === bot.botId"
                    >
                      重载
                    </n-button>
          ...
          <n-empty
            v-if="botListData.length === 0 && !loadingRef"
            description="暂无 Bot 实例，点击上方按钮添加"
  ...
  <n-modal
    ...
    negative-text="取消"
    :loading="creating"
    @positive-click="onCreateBot"
  >
    <n-space vertical>
      <n-form-item label="Bot ID" required>
        <n-input
          v-model:value="createForm.botId"
      ...
      <n-form-item label="Self ID">
        <n-input-number
          v-model:value="createForm.self_id"
          placeholder="Bot 的 QQ 号"
          style="width: 100%"
        />
      </n-form-item>
  ...
  <n-drawer v-model:show="configDrawerVisible" :width="560" placement="right">
    <n-drawer-content
      :title="`Bot 配置 - ${currentBot?.name || currentBot?.botId || ''}`"
      closable
    >
      <n-spin :show="configLoading">
        <n-space vertical>
          <n-form-item label="名称">
            <n-input
              v-model:value="configData.name"
              placeholder="Bot 显示名称"
            />
          </n-form-item>
          <n-form-item label="Self ID">
            <n-input-number
              v-model:value="configData.self_id"
              style="width: 100%"
            />
          </n-form-item>
          <n-form-item label="命令前缀">
          ...
          <template
            v-for="(adapter, index) in configData.adapters"
            :key="index"
          >
          ...
          <template
            v-if="!configData.adapters || configData.adapters.length === 0"
          >
...
async function handleStart(botId) {
  ...
  try {
    ...
    message.error(
      `启动失败: ${e.response?.data?.message || e.message || "未知错误"}`,
    );
  } finally {
...
async function handleStop(botId) {
  ...
  try {
    ...
    message.error(
      `停止失败: ${e.response?.data?.message || e.message || "未知错误"}`,
    );
  } finally {
    operatingBotId.value = null;
  }
...
async function onCreateBot() {
  ...
  try {
    ...
    message.error(
      `创建失败: ${e.response?.data?.message || e.message || "未知错误"}`,
    );
  } finally {
...
async function openConfig(bot) {
  ...
  try {
    ...
    configData.value = {
      ...
      prefix: Array.isArray(cfg.prefix)
        ? cfg.prefix
        : cfg.prefix
          ? [cfg.prefix]
          : ["/"],
