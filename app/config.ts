...
export function botConfigExists(botId: string): boolean {
    const botsDir = './config/bots';
    return fs.existsSync(path.join(botsDir, `${botId}.yml`)) || fs.existsSync(path.join(botsDir, `${botId}.yaml`));
}

export function createBotConfigFile(botId: string, config: { name?: string; self_id?: number; prefix?: string | string[]; adapters?: any[]; plugin_config?: any }): void {
    const botsDir = './config/bots';
