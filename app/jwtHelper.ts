import { generateKeyPairSync, KeyObject, createPrivateKey, createPublicKey, createSign, createVerify } from 'crypto';
...
function parseDurationSeconds(expiresInSeconds: number): number {
    return Number.isFinite(expiresInSeconds) && expiresInSeconds > 0 ? Math.floor(expiresInSeconds) : 0;
}
