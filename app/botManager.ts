import { listBotIds, botConfigExists, createBotConfigFile, deleteBotConfigFile, createBotConfig } from './config.js';
...
export class BotManager {
    ...
    async loadAll(): Promise<void> {
        ...
        logger.info(`[BotManager] Found ${botIds.length} bot configuration(s): ${botIds.join(', ')}`);
    ...
    listAllBots(): BotInfo[] {
        ...
        return allBotIds.map(botId => {
            ...
            return {
                ...
                pluginCount: 0,
                adapterCount: 0,
                prefix: Array.isArray(config.get('prefix')) ? config.get('prefix') : [config.get('prefix') || '/'],
            };
        });
    }

    async createInstance(botId: string, config: { name?: string; self_id?: number; prefix?: string | string[]; adapters?: any[]; plugin_config?: any }): Promise<BotInstance> {
        if (botConfigExists(botId)) {
            throw new Error(`Bot "${botId}" already exists`);
        }
        if (!/^[a-zA-Z0-9_-]+$/.test(botId)) {
            throw new Error('botId can only contain letters, numbers, underscores, and hyphens');
        }
