...
export class AdapterManager {
  ...
  private bot: Bot;
  private dbProvider: ((adapterId: string) => ScopedDatabaseClient) | null =
    null;
  ...
  async loadAdapter(type: string, adapterConfig: any): Promise<void> {
    ...
    const AdapterClass =
      module.default ||
      module[
        Object.keys(module).find((k) => k.endsWith("Adapter")) ||
          Object.keys(module)[0]
      ];
    if (!AdapterClass || typeof AdapterClass !== "function") {
      throw new Error(
        `Adapter "${type}" does not export a valid class. Found keys: ${Object.keys(module).join(", ")}`,
      );
    }
  ...
  async reloadAdapter(type: string): Promise<void> {
    ...
    const configuredAdapter = Array.isArray(configuredAdapters)
      ...
      : null;
    const adapterConfig =
      configuredAdapter?.config || this.adapterConfigs.get(type);
  ...
  async handleHttpRequest(req: any, res: any): Promise<boolean> {
    ...
    for (const adapter of this.adapters.values()) {
      if ("handleHttpRequest" in adapter) {
        const handled = await (adapter as IHTTPAdapter).handleHttpRequest(
          req,
          res,
        );
  ...
  watchAdapters(): void {
    ...
    this.watcher = fs.watch(
      ...
      (eventType, filename) => {
        ...
        if (!normalized.endsWith(".ts") && !normalized.endsWith(".json"))
          return;
  ...
  private static readAdapterMetadata(
    adapterDir: string,
    fallbackId: string,
  ): { id: string; name: string; defaultConfig: Record<string, any> } {
    ...
    try {
      ...
      const id =
        typeof metadata.id === "string" && metadata.id
          ...
          : fallbackId;
      const name =
        typeof metadata.name === "string" && metadata.name ? metadata.name : id;
      const defaultConfig =
        metadata.defaultConfig &&
        typeof metadata.defaultConfig === "object" &&
        !Array.isArray(metadata.defaultConfig)
          ? metadata.defaultConfig
          : {};
