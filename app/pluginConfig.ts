...
export function validateConfig(
  ...
  for (const field of schema) {
    let value =
      raw && field.propertyKey in raw ? raw[field.propertyKey] : field.default;

    if (value === undefined || value === null) {
      if (field.required) {
        errors.push(
          `Config field "${field.propertyKey}" is required but missing`,
        );
        continue;
      }
      value = field.default;
    ...
    try {
      ...
      errors.push(
        `Config field "${field.propertyKey}": ${(e as Error).message}`,
      );
      value = field.default;
    }
