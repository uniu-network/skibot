...
class ElysiaAdapterResponse {
    ...
    toResponse(): Response {
        ...
        return new Response(this.body == null ? null : String(this.body), { status: this.statusCode });
    }
}
...
export function createMessageRoutes(botManager: BotManager): AnyElysia {
    ...
    async function handle(instance: ReturnType<BotManager['getFirstInstance']>, body: unknown): Promise<Response> {
        if (!instance) {
            return new Response('No bot instances available', { status: 404 });
        }

        const res = new ElysiaAdapterResponse();
        const handled = await instance.adapterManager.handleHttpRequest({ body }, res);
        if (!handled) {
    ...
    router.post('/', async ({ body }) => handle(botManager.getFirstInstance(), body));
